...
`
export const CardBox = ({ title, children, className, headerComponent, isTabs }) => {
    ...
    >
        <CardHeader sx={{
            ...
            <div className="flex items-center gap-4">
                ...
                <h4 className="flex">  //{title} {state()?.terkait ? <span>RELATED TO <b>{state()?.terkait}</b></span> : ""} </h4>
            </div>
            {headerComponent}
        </div>} />
        {isTabs && <div className={`${mode() === "dark" ? "bg-[#0D0D0D]" : "bg-[#F5F5F5]"} flex `}>
            <div className="pt-4 flex flex-1 gap-[2px]">
                {state()?.tabs?.map((d, k) => {
                    let type = typeof d.display === "function" ? d.display().getProfile : d.display

                    return !type ? "" : <Link activeClass="current_active" inactiveClass="inActive" href={d.path}>
                        <Button onClick={() => {

                        }} class="!min-w-[150px]" color="primary" sx={{
