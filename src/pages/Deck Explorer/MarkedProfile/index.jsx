...
export const LayoutMarkedProfile = ({ children, title }) => {
  ...
  createEffect(() => {
    ...
    api()
      .get(`/deck-explorer/marked_profile?keyword=${id}`)
      .then((d) => {
        ...
        const keyMember =
          family_member.length !== 0 ? "family-member-detail" : null;
        const keyAlias = alias.length !== 0 ? "alias" : null;
        const keyPhone = phone_list.length !== 0 ? "msisdn" : null;
        ...
        const updatedTabs = [
          ...
          },
          {
            ...
            key: "msisdn",
            display: !!keyPhone,
            path: `/deck-explorer/marked-profile/${id}/msisdn`,
            dinamis: true,
          },
          {
            title: "VEHICLE",
        ...
        <div className="flex-1 flex flex-col">
          {items()?.tabs && (
            <CardBox
              title={title}
              isTabs
              className="relative xl:flex grid gap-4 flex-1 min-h-[600px]"
            >
              <div
                className={`min-w-[300px] col-span-1 xl:max-w-[280px] max-w-full border-r-2 ${
                  mode() === "dark" ? "border-[#0A0A0A]" : ""
                }`}
              >
                <div className="p-4 space-y-4">
                  <Paper
                    square
                    elevation={2}
                    class="relative w-full xl:h-[280px] h-[400px]"
                  >
                    {
                      <img
                        className="xl:object-cover object-contain object-center w-full h-full"
                        src={
                          items()?.getProfile?.foto_url?.label
                            ? items()?.getProfile?.foto_url.type === "file"
                              ? items()?.getProfile?.foto_url?.label
                              : items()?.getProfile?.foto_url?.label
                            : avatar
                        }
                  ...
                  <div
                    className={`space-y-2 ${
                      mode() === "dark" ? "text-white" : "text-[#444]"
                    }`}
                  >
                    <div>
                      <Tags className={"!py-0"} label={"Profile Name"} />
                      <p className="pl-4">
                        {items()?.getProfile?.profile_name}
                      </p>
                    </div>
                    <div>
                      <Tags className={"!py-0"} label={"CASE GROUP"} />
                      <p className="pl-[20px]">
                        {items()?.getProfile?.case_group}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="pl-4">
                        {moment(items()?.getProfile?.timestamp).format(
                          "D/M/Y || H:M"
                        )}{" "}
                        WIB
                      </p>
