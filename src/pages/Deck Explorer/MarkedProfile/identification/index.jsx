...
const PagesIdentity = () => {
  ...
  return (
    <LayoutMarkedProfile title={"IDENTIFICATION"}>
      <div className="flex-1 flex flex-col min-h-[600px] gap-4">
        <div className="w-full grid grid-cols-2 gap-2">
          <Button
            href={`#/deck-explorer/marked-profile/${path}/additional-info`}
            fullWidth
            variant="contained"
            color="secondary"
            startIcon={<Add></Add>}
          ...
          <Button
            href={`#/deck-explorer/marked-profile/${path}/identification/edit`}
            fullWidth
            variant="contained"
            color="secondary"
            startIcon={<Edit></Edit>}
          >
            EDIT PERSONAL INDETIFICATION
          </Button>
        </div>
          ...
          <div className="absolute left-0 w-full top-0 overflow-auto h-full grid grid-cols-2 gap-3 ">
            {items().loading ? (
              ...
              <>
                  ...
                  return (
                    <div
                      className={`bg-[#1e1e1e] p-2 ${
                        k === 0
                          ? " col-span-full"
                          : items()?.getProfile?.data?.length < 2
                          ? "col-span-full"
                          : ""
                      }`}
                    >
                ...
                {items()?.additional?.items?.length > 0 && (
                  <div className=" bg-[#1e1e1e] p-2">
                    <div className="border border-primarry-2 px-4 bg-primarry-1 sticky top-[5px] z-50 flex justify-between items-center">
                      <Tags
                        label={
                          <span>
                            DATA FROM <b>ADDITIONAL INFO</b>
                          </span>
                        }
                      ></Tags>
                    </div>
                            ...
                            <div
                              className={`${
                                mode() === "dark"
                                  ? "text-[#aaa]"
                                  : "text-[#444]"
                              } sticky top-[10px] whitespace-nowrap w-[220px] z-10`}
                            >
                              {d.category}
                            </div>
                            <FormControlLabel
