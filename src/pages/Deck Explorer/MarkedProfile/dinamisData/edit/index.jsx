...
const EditDinamis = () => {
    ...
    console.log(id)
    let idData = id === "getPhoneList" ? "phone_list" : id === "getVehicle" ? "kendaraan" : id === "getFamily" ? "family" : id === "getAlias" ? "alias" : ""
    ...
    createEffect(() => {
        api().get(`/deck-explorer/identification?keyword=${path}`).then(d => {
            ...
            let data = d.data.items[idData].map(w => {
                return {
                    ...w,
                    data: w.data.map(a => {
                        return {
    ...
    const onSubmit = (e) => {
        ...
        let data = {
            case_group, profile_name, remarks, keyword, [idData]: checkData()
        }
            ...
            <CardBox headerComponent={<div className="flex gap-4">
                ...
                <div className="flex gap-4 items-center">
                    <div className="space-x-4">
                        ...
                        {isEdit() && <Button variant="outlined" color="warning" onClick={() => {
                            setcancelEdit(d => !d)
                            setisEdit(false)
                                            ...
                                            return <div className={`flex gap-4 relative border-b ${mode() === "dark" ? "border-[#333]" : "border-[#aaa]"}  py-2 `}>
                                                <div className="flex gap-4 items-start flex-1 relative">
                                                                    ...
                                                                    <div className="pl-4 !m-0 border-[#454545] bg-[#2C2C2C] pr-2 py-1 flex gap-4 border-[0] max-w-sm">
                                                                        <div className="whitespace-nowrap max-w-xs relative">
                                                                            <div className="text-ellipsis overflow-hidden relative">
                                                                                {x.label !== "ID CARD PHOTO" ? <div className="gap-2 flex items-center">
                                                                                    {!d.field ?
                                                                                        <>
                                                                                            <IconButton color="info" size="small" onClick={() => onEdit(d.id, x.label)}>
                                                                                                <Edit fontSize="small"></Edit>
                                                                                            </IconButton>
                                                                                            {d.label}
                                                                                        </>
                                                                                        : <div>
                                                                                            <Input autoFocus defaultValue={d.label} sx={{ color: "white" }} onChange={(s) => {
                                                                                                setInputData(prev => ({ ...prev, [d.id]: s.target.value }))
                                                                                            }}></Input>
                                                                                            <IconButton size="small" color="info" onClick={() => {
                                                                                                onEdit(d.id)
                                                                                                let update = inputData()[d.id]
                                                                                                if (update) {
                                                                                                    onField(d.id, update)
                                                                                                }
                                                                                            }}>
                                                    ...
                                                    <div className="flex items-center justify-center h-full">
                                                        <Button size="small" color="error" onClick={() => { removeId(x.id, b.id, x.label) }}><Delete fontSize="small"></Delete></Button>
                                                    </div>
                                                </div>
